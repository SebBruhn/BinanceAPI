#In this file we will create a Python class

#Our program is a client and we are going to requst data
#from a server using the REST API

from typing import *
import logging

import requests

#Instantiate a logger

logger = logging.getLogger()



class BinanceClient:
    def __init__(self, futures=False):
        

        self.futures = futures

        if self.futures:
            self._base_url = "https://fapi.binance.com"
        else:
            self._base_url = "https://api.binance.com"

        #create a variable self.symbols that will store the output of _get_symbols method
        #self.symbols will be called whenever we instantiate the class, since we place it in the init method.
        self.symbols = self._get_symbols()

        #Taking as an argument a dictionary.
    def _make_requests(self, endpoint: str, query_parameters: Dict):

        """
        Creating a variable to store the result of the request. That will be
        called "response".
        """

        try:
            response = requests.get(self._base_url + endpoint, params = query_parameters)
        except Exception as e:
            logger.error("Connection error while making request to %s: %s", endpoint, e)
            return None
        """
        The status code can be 200 if the request is successful, then we can return the JSON reponse from it.
        """

        """
        You insert variables in your log by writing %s 
        """


        if response.status_code == 200:
            return response.json()
        else:
            logger.error("Error while making requests to %s: %s (status code = %s) ",
                          endpoint, response.json(), response.status_code)
            return None

        """
        Method to get a list of contracts that will be symbols
        """

    def _get_symbols(self) -> List[str]:
            
            #Variable to store the parameters
            params = dict()

            endpoint = "/fapi/v1/exchangeInfo" if self.futures else "/api/v3/exchangeinfo"
            #We call the self._make_request() method passing the endpoint and these parameters
            data = self._make_requests(endpoint, params)

            symbols = [x["symbol"] for x in data["symbols"]]

            print(symbols)

            return symbols
    
    def get_historical_data(self, symbol: str, start_time: Optional[int] = None, end_time: Optional[int] = None):

        params = dict()

        params["symbol"] = symbol
        params["interval"] = "1m"
        params["limit"] = 1500

        if start_time is not None:
            params["startTime"] = start_time
        if end_time is not None:
            params["endTime"] = end_time
        
        endpoint = "/fapi/v1/klines" if self.futures else "/api/v3/klines"
        raw_candles = self._make_requests(endpoint, params)

        candles = []

        if raw_candles is not  None:
            for c in raw_candles:
                candles.append((float(c[0]), float(c[1]), float(c[2]), float(c[3]), float(c[4]), float(c[5]),))
            return candles
        else:
            return None
        

